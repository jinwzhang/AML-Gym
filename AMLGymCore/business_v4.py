...
class Business(Env):
    ...
    def __init__(self, train_memo='default_'):
        super().__init__()
        self.train_memo = train_memo

        self.ingestion = conf.INGENSTION
        self.aml_policy = conf.AML_POLICY
        self.aml_random_check = conf.AML_RAMDOM_CHECK
        ...
        self.trade_config = trade_table_preprocess(trade_config_df, conf.GROUP).copy()
        ...
        self.trade_config['_products'] = self.trade_config['product_name'].apply(multi_target_loader)
        ...
        self.asset_holds_obs = combine_dict_keys(i for i in self.trade_config['_products'].copy().tolist())
    ...
    def observation_space(self):
        VA = conf.TRANSACTION_VALUE
        asset_holds = {k: math.log(1/math.e + v/VA) for k, v in self.asset_holds.items()}
    ...
    def reset(self, **kwargs):
        ...
        if rd.random() > 0.98 or self.reward >= self.max_reward > 200:
            self.record_keeper_df.to_excel(self.log_path + 'record_keeper_log_at_step%d_reward%d.xlsx' % (self.N, self.reward))
    ...
    def _do_trade(self, trade_id):
        ...
        new_state = self.observation_space
        if actual_amount > 5:
        ...
        done = False if (self.n < self.max_steps and not self.arrested) else True
        params = {'product': actual_paid, 'actual_amount': actual_amount, 'trade_object': trade_object,
                  ...
                  'done': done, 'additional_asset_type': additional_asset_type, 'arrested': self.arrested, 'n': self.n}
